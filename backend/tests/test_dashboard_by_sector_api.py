...
class DashboardBySectorApiTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        try:
            ...
            raise unittest.SkipTest(f"dashboard by sector integration tests require local Postgres/Redis: {exc}") from exc
    ...
    def _prepare_database(cls) -> None:
        ...
        try:
            ...
            OrganizationImportService(OrganizationImportRepository(db)).import_file(
                ...
                source_name="dashboard_by_sector_fixture",
                dry_run=False,
            )
    ...
    def _cache_key(self, **filters) -> str:
        ...
        return DashboardBreakdownService.build_cache_key(
            DashboardBreakdownDimension.PRIMARY_INDUSTRY_SECTOR,
            params,
        )

    def test_dashboard_by_sector_returns_breakdown_sorted_by_count_then_code(self) -> None:
        cache_key = self._cache_key()
        get_redis().delete(cache_key)

        response = self.client.get("/api/dashboard/by-sector")

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertTrue(payload["success"])
        self.assertEqual(payload["meta"]["group_by"], "primary_industry_sector")
        ...
        self.assertEqual(
            [bucket["primary_industry_sector_code"] for bucket in buckets],
            [
    ...
    def test_dashboard_by_sector_applies_filters(self) -> None:
        cache_key = self._cache_key(
        ...
        self.assertEqual(payload["data"][0]["primary_industry_sector_code"], "manufacturing")
        self.assertEqual(payload["data"][0]["organization_count"], 1)
        self.assertEqual(payload["data"][0]["mappable_count"], 1)

    def test_dashboard_by_sector_uses_redis_cache(self) -> None:
        cache_key = self._cache_key()
        redis = get_redis()
        redis.delete(cache_key)

        first_response = self.client.get("/api/dashboard/by-sector")
        ...
        try:
            db.execute(
                ...
                SET primary_industry_sector_id = (
                    SELECT id FROM industry_sectors WHERE code = 'education_and_training'
                )
                WHERE external_code = 'ORG-CAT-001'
                """
            )

            cached_response = self.client.get("/api/dashboard/by-sector")
            self.assertEqual(cached_response.status_code, 200)
            cached_payload = cached_response.json()
            self.assertTrue(cached_payload["meta"]["cache_hit"])
            cached_counts = {
                bucket["primary_industry_sector_code"]: bucket["organization_count"]
                for bucket in cached_payload["data"]
            }
            self.assertEqual(cached_counts["education_and_training"], 1)
            self.assertEqual(cached_counts["manufacturing"], 1)

            redis.delete(cache_key)

            uncached_response = self.client.get("/api/dashboard/by-sector")
            ...
            uncached_counts = {
                bucket["primary_industry_sector_code"]: bucket["organization_count"]
                for bucket in uncached_payload["data"]
            }
            self.assertEqual(uncached_counts["education_and_training"], 2)
            self.assertNotIn("manufacturing", uncached_counts)
        finally:
            db.execute(
                """
                UPDATE organizations
                SET primary_industry_sector_id = (
                    SELECT id FROM industry_sectors WHERE code = 'manufacturing'
                )
                WHERE external_code = 'ORG-CAT-001'
                """
            )
    ...
    def test_dashboard_by_sector_falls_back_when_redis_cache_fails(self) -> None:
        ...
        self.assertFalse(payload["meta"]["cache_hit"])
        self.assertEqual(payload["meta"]["matched_total"], 1)
        self.assertEqual(payload["data"][0]["primary_industry_sector_code"], "education_and_training")
        self.assertEqual(payload["data"][0]["organization_count"], 1)
